...
#Importing Data
election = pd.read_csv("C:/data science/logistic regression/Datasets_LR/election_data.csv", sep = ",")
election.info()
#removing CASENUM

c1=election.drop(["Election-id",'Year'],axis=1)
c1.info()
c1.describe()
c1.isna().sum()
...
c1.rename({'Popularity Rank': 'Popu', 'Amount Spent': 'Amsp'}, axis=1, inplace=True)

#Model building 
# import statsmodels.formula.api as sm
logit_model = sm.logit('Result ~ Amsp + Popu',data=c1).fit()
...
train_data, test_data = train_test_split(c1, test_size = 0.30) # 30% test data

# Model building 
# import statsmodels.formula.api as sm

model = sm.logit("Result ~ Amsp + Popu +pred", data = train_data).fit()
