...
#Importing Data
ads = pd.read_csv("C:/data science/logistic regression/Datasets_LR/advertising.csv", sep = ",")

#removing CASENUM
c1 = ads.drop(['Timestamp',"Country","City","Ad_Topic_Line"], axis = 1)
...
cols=['Daily_Time_ Spent _on_Site', 'Age', 'Area_Income','Daily Internet Usage', 'Male']
c1.info()
c1.describe()
c1.isna().sum()
...
c1.rename({'Daily Internet Usage': 'DIU', 'Daily_Time_ Spent _on_Site': 'DIS'}, axis=1, inplace=True)

#Model building 
# import statsmodels.formula.api as sm
logit_model = sm.logit("Clicked_on_Ad~ DIS + Age + Area_Income + DIU + Male", data=c1).fit()

#summary
logit_model.summary2() # for AIC
logit_model.summary()

pred = logit_model.predict(c1.iloc[ :, 0:5 ])
...
model = sm.logit("Clicked_on_Ad~ DIS + Age + Area_Income + DIU + Male", data = train_data).fit()
